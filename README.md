...
-   **🏷️ 标签筛选** - 47 个精准分类标签，通过 GitHub Labels 管理，支持双向同步
...
| **sync-labels**               | Label 变更时自动触发      | 从 GitHub Labels 同步到本地    | ~1 分钟     |
...
#### GitHub Labels 同步（新功能）

标签现在通过 GitHub Labels 管理，支持双向同步：

- **管理标签**：在 GitHub 仓库的 Labels 页面增删改 `分类:xxx` 标签
- **自动同步**：Label 变更后自动同步到 `bookmarks.json`
- **新工具收录**：提交 Issue 时如果使用新标签，会自动创建对应的 GitHub Label

**相关工作流**：
- `🔄 sync-labels` - 从 GitHub Labels 同步到本地（Label 变更时自动触发）

**标签前缀规则**：
- 分类标签：`分类:xxx`（会同步到网站）
- 系统标签：无前缀（如"待审核"、"已收录"，不会同步）
...
**核心功能**：状态管理、数据校验、动画效果、图表可视化、流程图、富文本编辑、代码编辑、终端模拟、视频播放、音频处理、图片处理、图片生成、文件上传、文件解析、文件检测、拖拽排序、滚动交互、手势识别、网络请求、实时协作、日期时间、数学计算、公式渲染、国际化、安全防护、本地存储、剪贴板、快捷键、用户引导

**UI组件**：图标库、颜色选择、轮播组件、日历组件、提示组件、签名组件、虚拟键盘、加载动画、JSON编辑

**工具类**：工具函数、唯一标识、模糊搜索、模拟数据、编码转换、DOM操作、演示文稿
