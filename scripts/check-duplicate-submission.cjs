...
function checkDuplicateSubmission(issueBody) {
    ...
    if (!urlMatch) {
        console.error('⚠️ 未找到 GitHub URL');
        return result;
    }
    ...
    try {
        ...
        for (const bookmark of bookmarksData.bookmarks) {
            ...
            if (
                existingUrl.includes(submittedUrl) ||
                submittedUrl.includes(existingUrl.replace('https://github.com/', ''))
            ) {
                result.isDuplicate = true;
                result.existingTitle = bookmark.title;
                console.error(`❌ 检测到重复提交: ${bookmark.title}`);
                break;
            }
        }

        if (!result.isDuplicate) {
            console.error('✅ 未检测到重复');
        }
    } catch (error) {
