...
function FavoritesHero({ favoritesCount }: FavoritesHeroProps) {
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400">
                {favoritesCount > 0
                    ? `你已收藏 ${favoritesCount} 个工具，快速访问你喜欢的工具库`
                    : '还没有收藏任何工具，快去首页添加吧！'}
            </p>
        </div>
    );
...
export default function Favorites() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex flex-col">
            ...
            <FavoritesHero favoritesCount={favorites.length} />

            <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12 w-full">
                {favoriteBookmarks.length > 0 ? (
                    <BookmarkList bookmarks={favoriteBookmarks} viewMode="list" getTagColor={getTagColor} onShare={handleShare} />
                ) : (
                    <EmptyFavoritesState />
                )}
            </main>
