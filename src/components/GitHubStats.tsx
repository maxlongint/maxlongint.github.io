...
export default function GitHubStats({ url }: GitHubStatsProps) {
    ...
    return (
        <div className="flex items-center gap-1.5 flex-shrink-0">
            {repoInfo.pushed_at && (
                <a
                    href={`https://github.com/${repoInfo.full_name}/commits`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-xs text-gray-500 hover:text-gray-700 transition-colors whitespace-nowrap"
                    title={new Date(repoInfo.pushed_at).toLocaleString('zh-CN')}
                >
                    更新于 {formatUpdateTime(repoInfo.pushed_at)}
                </a>
            )}
            <a
            ...
            >
                <img
                    src={`https://img.shields.io/github/stars/${repoInfo.full_name}?style=social`}
